Fujicoin Core version 0.19.0.1 is now available from:

  <https://fujicoincore.org/bin/fujicoin-core-0.19.0.1/>
...
Fujicoin Core is supported and extensively tested on operating systems using
the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not recommended
to use Fujicoin Core on unsupported systems.
...
to open the Fujicoin Core .dmg. This is due to new signing requirements
imposed by Apple, which the Fujicoin Core project does not yet adhere too.
...
- [Reduce memory](https://github.com/fujicoin/fujicoin/blob/master/doc/reduce-memory.md)
  suggests configuration tweaks for running Fujicoin Core on systems with
...
- `-blockfilterindex` enables the creation of BIP158 block filters for
  ...
  currently use about 4 GiB of space.  Note: this version of Fujicoin
...
- `whitebind` and `whitelist` now accept a list of permissions to
  ...
  releases.  See the `fujicoind -help` output for these two options for
...
- In 0.18.0, a `./configure` flag was introduced to allow disabling BIP70 support in the GUI (support was enabled by default). In 0.19.0, this flag is now __disabled__ by default. If you want to compile Fujicoin Core with BIP70 support in the GUI, you can pass `--enable-bip70` to `./configure`. (#15584)
...
- `generate` is now removed after being deprecated in Fujicoin Core 0.18.
...
- BIP 61 reject messages were deprecated in v0.18. They are now disabled
  ...
  future version of Fujicoin Core. (#14054)

- To eliminate well-known denial-of-service vectors in Fujicoin Core,
  ...
  This prevents Fujicoin Core from sending the BIP111 NODE_BLOOM service
  ...
  of Fujicoin Core and nodes that have manually enabled BIP37 support,
...
- By default, Fujicoin Core will now make two additional outbound connections that are exclusively used for block-relay.  No transactions or addr messages will be processed on these connections. These connections are designed to add little additional memory or bandwidth resource requirements but should make some partitioning attacks more difficult to carry out. (#15759)

Miscellaneous CLI Changes
-------------------------

- The `testnet` field in `fujicoin-cli -getinfo` has been renamed to
...
- `getblockchaininfo` no longer returns a `bip9_softforks` object.
  ...
  an additional `type` field describes how Fujicoin Core determines
...
- When fetching a transaction announced by multiple peers, previous versions of
  Fujicoin Core would sequentially attempt to download the transaction from each
...
- If a Tor hidden service is being used, Fujicoin Core will be bound to
...
- Transactions with outputs paying v1 to v16 witness versions (future
  ...
  and services can accept any valid bech32 Fujicoin address without
...
- When creating a transaction with a fee above `-maxtxfee` (default 0.1
  FJC), the RPC commands `walletcreatefundedpsbt` and
  `fundrawtransaction` will now fail instead of rounding down the fee.
  Be aware that the `feeRate` argument is specified in FJC per 1,000
...
- A new wallet flag `avoid_reuse` has been added (default off). When
  ...
  partial spends" (added in Fujicoin Core v0.17.0), this can eliminate a
...
- #15649 Add ChaCha20Poly1305@Fujicoin AEAD (jonasschnelli)
...
- #15639 fujicoin-wallet tool: Drop libfujicoin_server.a dependency (ryanofsky)
...
- #16521 Use the default maxfeerate value as FJC/kB (Remagpie)
...
- #16366 init: Use InitError for all errors in fujicoind/qt (MarcoFalke)
...
- #15239 scripts and tools: Move non-linux build source tarballs to "fujicoin-binaries/version" directory (hebasto)
...
- #16258 [MSVC]: Create the config.ini as part of fujicoind build (NicolasDorier)
...
- #16441 remove qt libjpeg check from fujicoin_qt.m4 (fanquake)
...
- #16750 msbuild: adds bench_fujicoin to auto generated project files (sipsorcery)
...
- #15773 Add FujicoinTestFramework::sync_* methods (MarcoFalke)
...
- #15352 Reduce noise level in test_fujicoin output (practicalswift)
...
- #15896 feature_filelock, interface_fujicoin_cli: Use PACKAGE_NAME in messages rather than hardcoding Fujicoin Core (luke-jr)
...
- #16277 Suppress output in test_fujicoin for expected errors (gertjaap)
...
- #16929 follow-up to rpc: default maxfeerate value as FJC/kB (jonatack)
...
- #16489 log: harmonize fujicoind logging (jonatack)
...
- #17449 fix uninitialized variable nMinerConfirmationWindow (fujicoinVBR)

### Documentation
- #15514 Update Transifex links (fanquake)
- #15513 add "sections" info to example fujicoin.conf (fanquake)
...
- #15683 Comment for seemingly duplicate LIBFUJICOIN_SERVER (Bushstar)
...
- #16080 build/doc: update fujicoin_config.h packages, release process (jonatack)
...
- #16448 add note on precedence of options in fujicoin.conf (fanquake)
...
- #16621 add default fujicoin.conf locations (GChuf)
...
- #16863 Add a missing closing parenthesis in the fujicoin-wallet's help (darosior)
- #16757 CChainState return values (MarcoFalke)
- #16847 add comments clarifying how local services are advertised (jamesob)
- #16812 Fix whitespace errs in .md files, fujicoin.conf, and Info.plist.in (ch4ot1c)
...
- #16912 Remove Doxygen intro from src/fujicoind.cpp (ch4ot1c)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/fujicoin/fujicoin/).
