...
Fujicoin Core version 0.20.0 is now available from:

  <https://fujicoincore.org/bin/fujicoin-core-0.20.0/>
...
#### Removal of BIP61 reject network messages from Fujicoin Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Fujicoin Core version 0.18.0.
...
- Testing or debugging of implementations of the Fujicoin P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Fujicoin Core. Fujicoin Core logs debug messages
...
- OpenSSL is no longer used by Fujicoin Core.  (#17265)

- BIP70 support has been fully removed from Fujicoin Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/fujicoin/fujicoin/issues/16599),
  [PR #16702](https://github.com/fujicoin/fujicoin/pull/16702), and the
  `fujicoind help` for more information.  This option is experimental and
...
- All custom settings configured when Fujicoin Core starts are now
...
- The "Start Fujicoin Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://fujicoincore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple Fujicoin Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Fujicoin Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/fujicoin-core/HWI) for
  signing.  Future versions of Fujicoin Core should support a GUI option
...
- Fujicoin Core's automatically-generated source code documentation is
  now available at https://doxygen.fujicoincore.org.  (#17596)
...
- The `fujicoin-cli` utility used with the `-getinfo` parameter now
...
- The `fujicoin-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `fujicoinstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_fujicoin-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBFUJICOIN_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_fujicoin` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBFUJICOIN_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for fujicoin-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate fujicoin schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest fujicoin.conf (sdaftuar)
...
- #18022 Fix appveyor `test_fujicoin` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (fujicoin-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone fujicoin-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Fujicoin Core files (hebasto)
...
- #17119 Fix broken fujicoin-cli examples (andrewtoth)
...
- #17503 Remove bitness from fujicoin-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.fujicoincore.org (laanwj)
...
- #17648 Rename wallet-tool references to fujicoin-wallet (hel-o)
...
- #18341 Replace remaining literal FJC with `CURRENCY_UNIT` (domob1812)
...
- #16115 On fujicoind startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from fujicoin-cli -getinfo (malevolent)
