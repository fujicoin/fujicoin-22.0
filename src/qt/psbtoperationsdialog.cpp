...
void PSBTOperationsDialog::saveTransaction() {
    ...
    for (const CTxOut& out : m_transaction_data.tx->vout) {
        ...
        QString amount = FujicoinUnits::format(m_wallet_model->getOptionsModel()->getDisplayUnit(), out.nValue);
...
std::string PSBTOperationsDialog::renderTransaction(const PartiallySignedTransaction &psbtx)
{
    ...
    for (const CTxOut& out : psbtx.tx->vout) {
        ...
        tx_description.append(tr(" * Sends %1 to %2")
            .arg(FujicoinUnits::formatWithUnit(FujicoinUnits::FJC, out.nValue))
    ...
    if (!*analysis.fee) {
        ...
        tx_description.append(FujicoinUnits::formatWithUnit(FujicoinUnits::FJC, *analysis.fee));
