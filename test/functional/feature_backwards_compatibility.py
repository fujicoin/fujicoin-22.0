...
class BackwardsCompatibilityTest(FujicoinTestFramework):
    ...
    def run_test(self):
        ...
        if self.is_bdb_compiled():
            ...
            # Copy the 0.16 wallet to the last Fujicoin Core version and open it:
            shutil.copyfile(
                os.path.join(node_v16_wallets_dir, "wallets/u1_v16"),
                os.path.join(node_master_wallets_dir, "u1_v16")
            )
            load_res = node_master.loadwallet("u1_v16")
            # Make sure this wallet opens without warnings. See https://github.com/fujicoin/fujicoin/pull/19054
